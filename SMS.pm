...
sub telephonySmsList
{
    ...
    return apiRequest({ reqUrl => '/sms/sms-list' , reqType => 'POST', req => $req });
}
...
sub telephonySmsDelete
{
    ...
    return apiRequest({ reqUrl => '/sms/delete-sms' , reqType => 'POST', req => $req  });
}
...
sub telephonySmsSend
{
	...
	{
		$Result->{'status'} 	= "201";
        $Result->{'message'} 	= 'Destination number must be a mobile phone in France !';
        return $Result;
	}
	...
	# Make rquest
	return apiRequest({ reqUrl => '/sms/send-sms' , reqType => 'POST', req => $req });
}
